...
void main(void) {
    // Variables
    char display[4] = {'0', '0', '0', '0'}; // Initialize to "0000"
    ...
    // Configure P1.7 as ADC input (AIN7, mapped to ADC channel 0)
    AINDIDS |= (1 << 7); // Enable AIN7 (P1.7)
    P1M1 |= (1 << 7);    // Set P1.7 as input
    P1M2 &= ~(1 << 7);

    // Configure ADC for P1.7 (12-bit, channel 0)
    ADCCON1 |= 0x01;       // Enable ADC
    ADCCON0 &= ~0x0F;      // Clear channel bits
    ADCCON0 |= 0x00;       // Select channel 0 (AIN7)
    ADCCON1 &= ~(1 << 1);  // Ensure single conversion mode
    ADCCON2 |= (1 << 3);   // Set 12-bit resolution (ADCDIV=0, 12-bit mode)
    ...
    while (1) {
        ...
        adc_value = (unsigned int)((ADCRH << 4) | (ADCRL >> 4)); // Combine 12-bit result

        // Validate ADC value
        if (adc_value > 4095) {
            adc_value = 4095; // Cap at max 12-bit value
        }

        // Extract digits for display (0–4095)
        display[0] = '0' + (adc_value / 1000);          // Thousands (0–4)
        display[1] = '0' + ((adc_value % 1000) / 100);  // Hundreds (0–9)
        display[2] = '0' + ((adc_value % 100) / 10);   // Tens (0–9)
        display[3] = '0' + (adc_value % 10);           // Units (0–9)

        // Validate display digits
        if (display[0] < '0' || display[0] > '4') {
            display[0] = '0'; // Ensure thousands digit is 0–4
        }
        for (i = 1; i < 4; i++) {
